...
<body>
  ...
  <h1>Search Booking Details/Delete Booking</h1>
  <form method="POST" action="">
    <input type="text" name="bookingId" placeholder="Search by booking ID">
    <button type="submit" name="searchBookingId">Search by Booking ID</button>
  </form>
  <h1>or</h1>
  <form method="POST" action="">
  ...
  // Check if the search by booking ID form is submitted
  if (isset($_POST['searchBookingId'])) {
    $bookingId = $_POST['bookingId'];

    // Query to search for information in the database by booking ID
    ...
    if ($result->num_rows > 0) {
      ...
      while ($row = $result->fetch_assoc()) {
        ...
        echo '<button type="button" onclick="deleteRow(' . $row['booking_id'] . ')">Delete</button>';
        echo "<hr>";
      }
      echo '</div>';
    } else {
      ...
      if (confirm("Are you sure you want to delete this row?")) {
        ...
        xhr.onreadystatechange = function () {
          if (xhr.readyState == 4 && xhr.status == 200) {
            // Reload the page to update the results
            location.reload();
          }
        };
